...
All notable changes to the Clawey agent will be documented in this file.
...
### Android Mobile App (ClaweyApp)

Full React Native / Expo SDK 55 Android app built from scratch — enables controlling Clawey from anywhere via mobile.

#### New: Android App (ClaweyApp/)
...
#### Files Changed
- **New**: `ClaweyApp/` — entire React Native app directory
- **New**: `ClaweyApp/services/push-notifications.ts` — FCM token registration, notification categories
- **New**: `ClaweyApp/services/voice.ts` — expo-audio recording + Gemini STT upload
- **New**: `ClaweyApp/services/socket.ts` — Socket.io client wrapper with Zustand integration
- **New**: `ClaweyApp/components/orgs/ProposalsView.tsx` — proposals UI
- **New**: `ClaweyApp/components/orgs/BlockersView.tsx` — blockers UI
- **New**: `ClaweyApp/components/orgs/MemoryView.tsx` — memory browser UI
- **Updated**: `ClaweyApp/app/_layout.tsx` — push notification init + deep-link handler
- **Updated**: `ClaweyApp/app/org/[orgId].tsx` — 5-tab org detail screen
- **Updated**: `ClaweyApp/constants/index.ts` — DEFAULT_SERVER_URL → Cloudflare URL
...
- **Updated**: `ClaweyApp/eas.json` — production build + submit config
...
- Create orgs via Dashboard UI form or Clawey chat (`manage_org` skill)
...
### Clawey Browser Relay Extension

New Chrome MV3 extension that bridges Clawey to the user's real Chrome tabs — no `--remote-debugging-port` flag needed.
...
# 2. Extension auto-connects to Clawey (ws://127.0.0.1:3000/relay)
...
Clawey can now connect to the user's **real running Chrome session** — not a new instance, but the actual browser with all real logins, cookies, and active tabs.
...
- **Workspace Declutter**:
  - Deleted obsolete root-level documentation exports (`Clawey_Overview.md`, `Clawey_Overview.pdf`, etc.) and temporary images.
...
## [1.17.0] - 2026-03-14
### Added
- **🧬 Self-Learning Engine**: Clawey now passively learns from every conversation.
...
## [1.16.0] - 2026-03-14
### Added
- **🔄 Model Failover Chain**: Clawey now cascades through 5 models automatically on failure:
...
- **🧠 Brain Overhaul**: Completely rebuilt the AI reasoning engine and system prompt from the ground up.
  - **Identity & Personality**: Clawey now has a defined personality — direct, efficient, technically sharp with dry humor.
...
## [1.12.0] - 2026-03-13
### Added
- **🧠 Continuous Learning Engine**: Implemented `manage_long_term_memory` skill, allowing Clawey to learn user preferences, shorthand, and custom MSP workflows across sessions.
...
- **Brand Identity**: Generated a futuristic logo for Clawey and integrated it into the documentation assets.
...
## [1.4.0] - 2026-03-12
### Added
- **Browser Relay Extension**: Built a custom Chrome extension to allow Clawey to control the user's active browser tabs.
...
## [1.0.0] - 2026-03-11
### Added
- Initial release of Clawey agent.
