# Clawey v12 — Final Implementation Plan
...
| Decision | Answer |
|---|---|
| Org creation | Both — Dashboard UI form + Clawey chat commands |
...
v11 gave Clawey a multi-brain human conversation system with parallel workers. v12 adds a completely separate layer that runs **above** that system — autonomous AI organisations that operate on their own schedules, manage their own tasks, and work on real project directories without any human in the loop.
...
src/core/
  ...
  org-management-skill.ts   ← Skill for Clawey chat-based org management
...
```json
{
  ...
  "name": "Clawey Dev Team",
  "mission": "Build and maintain Clawey as the world's best local-first AI automation platform.",
  "rootDir": "C:/Projects/Clawey",
...
| FIX-F | 🟡 | Clawey chat-based org creation — Brain has no direct access to OrgManager | `manage_org` skill registered globally — Brain calls it as a tool. Also add slash command handlers in `index.ts` |
...
private initSession() {
  ...
  this.history = [
    ...
    { role: 'model', parts: [{ text: 'Online. Clawey v11 is ready. What do you need?' }] },
  ];
  this.turnCount = 0;
...
Registered globally — allows Clawey's regular Brain to manage orgs via chat.
...
export const orgManagementSkill: Skill = {
  name: 'manage_org',
  description: `Manage Clawey AI organisations and their agents. Actions:
      ...
      <div className="modal-panel modal-panel--wide" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Clawey Dev Team" />
          </div>
          <div className="form-group">
...
#### New Clawey Skill (all chat panes)
...
79. **Org management via chat CRUD** — "list my orgs" → "pause the Clawey org" → "trigger Maya's heartbeat" → "add a new agent to org X" → all work via tool call
...
19. **`manage_org` skill registered in global skills index** — available to Clawey's regular Brain.
