# Clawey v12.1 — Patch: Multi-Org Protection System
...
The original v12.1 plan ran `git ls-files` from `process.cwd()` (Clawey's
...
- Works for public users who download Clawey and point it at any project
...
export function snapshotGitFiles(rootDir: string): string[] {
  try {
    const output = execSync('git ls-files', {
      cwd: rootDir,           // FIX: org's dir, not Clawey's dir
      encoding: 'utf-8',
      timeout: 5000,
    });
    return output.split('\n')
...
export function CreateOrgModal({ onSubmit, onClose }: CreateOrgModalProps) {
  ...
  const browsePath = async () => {
    // Clawey runs on Windows. Use PowerShell via the backend to open a folder picker.
    try {
      ...
      <div className="modal-panel modal-panel--wide" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Clawey Enterprise" />
          </div>
          <div className="form-group">
...
6. Start server. Verify:
   - `POST /api/check-git` with Clawey dir → `{ available: true, fileCount: N }`
   - `POST /api/check-git` with non-git dir → `{ available: false, fileCount: 0 }`
   - Create org with `protectionMode: 'git'` → `org.protection.gitFiles` contains paths from that org's root dir, not Clawey's
...
12. **Git protection** — create Clawey org with Git mode → `protectedFiles` contains Clawey files only
13. **Separate org git protection** — create MSP Genie org pointing at MSP Genie dir → `protectedFiles` contains MSP Genie files, zero overlap with Clawey
...
21. **Multi-org isolation** — Clawey org protected files ≠ MSP Genie protected files — confirmed in `org.json` for each
