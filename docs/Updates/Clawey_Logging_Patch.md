...
| System | Before | After |
|---|---|---|
| Terminal output | Printed, lost on restart | Written to `logs/clawey-{date}.log`, 7-day rolling |
...
│   ├── clawey-2026-03-18.log     ← terminal output for that day
│   ├── clawey-2026-03-19.log
...
```typescript
/**
 * Clawey Terminal Logger
 *
 * Tees all console output (log/warn/error) to a rolling daily log file.
 * Files live in logs/clawey-{date}.log, auto-deleted after 7 days.
...
class TerminalLogger {
  ...
  start(): void {
    ...
    // Log startup marker
    const marker = `\n${'='.repeat(60)}\n  Clawey started at ${new Date().toISOString()}\n${'='.repeat(60)}\n`;
  ...
  private rotateIfNeeded(): void {
    ...
    const file = path.join(LOGS_DIR, `clawey-${today}.log`);
  ...
  private cleanup(): void {
    try {
      ...
      for (const f of fs.readdirSync(LOGS_DIR)) {
        if (!f.startsWith('clawey-') || !f.endsWith('.log')) continue;
...
### `logs/clawey-2026-03-18.log`

```
============================================================
  Clawey started at 2026-03-18T09:00:00.123Z
============================================================
[2026-03-18T09:00:00.456Z] [INFO] [Server] Initializing Clawey v12...
...
[2026-03-18T09:00:05.678Z] [INFO] [OrgHeartbeat] ⏰ Heartbeat: Aria (CEO) in Clawey Enterprise
...
### Integration Tests
10. **Terminal log created** — start server → `logs/clawey-{today}.log` exists
