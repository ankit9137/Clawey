# Clawey v12.8 — Complete Setup Guide

Everything you need to get Clawey running on your machine, including the optional Android app and remote access.
...
### Clone & Setup
```bash
git clone https://github.com/ankit9137/Clawey.git
cd Clawey
setup.bat
```
...
The extension lets Clawey interact with your real Chrome tabs (not just its own Playwright browser).
...
5. Type `/relay` in Clawey chat to verify

---

## 3. Telegram Bot (Optional)

Control Clawey from your phone via Telegram — useful for quick commands when you're away from your desk.
...
Full mobile app for controlling Clawey from your phone — chat, manage orgs, push notifications, voice input.
...
2. Add an **Android app** with package name `com.clawey.app`
3. Download `google-services.json` → place it in `ClaweyApp/android/app/`
...
4. **Create a tunnel**:
   ```bash
   cloudflared tunnel create clawey
   ```
   Note the **Tunnel ID** from the output.

5. **Add a DNS record**:
   ```bash
   cloudflared tunnel route dns clawey api.yourdomain.com
   ```
...
### Verify
- Visit `https://api.yourdomain.com/status` in a browser — you should see the Clawey status JSON
...
| `SDK location not found` | Create `ClaweyApp/android/local.properties` with `sdk.dir=C\:\\Users\\<you>\\AppData\\Local\\Android\\Sdk` |
