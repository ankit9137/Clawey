...
const ANALYSIS_PROMPT = `You are a conversation analyst for an AI assistant called Clawey. Your job is to analyze a conversation and extract learnable insights about the USER (not the AI).
...
export class Learner {
  ...
  public queueAnalysis(conversationHistory: any[]) {
    ...
    const userTurns = conversationHistory.filter(
      (h: any) => h.role === 'user' && !h.parts?.[0]?.text?.startsWith('# Clawey')
    );
