/**
 * Clawey Extension Relay — WebSocket server for browser extension communication.
 *
 * The relay bridges the Chrome extension and the Clawey brain/skills.
...
export class ExtensionRelay {
  ...
  async executeCommand(command: string, params: any = {}, timeoutMs = 30000): Promise<any> {
    if (!this.client || this.client.readyState !== WebSocket.OPEN) {
      throw new Error('Extension not connected. Install and enable the Clawey Relay extension in Chrome.');
    }
