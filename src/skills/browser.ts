...
/**
 * Unified browser skill — the ONLY browser tool for Clawey.
 *
 * THREE MODES:
 ...
 *   • extension-relay — connected via the Clawey Chrome extension for real DOM access.
 *
...
EXTENSION RELAY (Clawey Chrome Extension):
  ...
  run: async ({ action, url, target, text, code, port, args, tab_id }: {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'relay_tabs': {
          if (!extensionRelay.connected) return { success: false, error: 'Extension not connected. Install the Clawey Relay extension in Chrome.' };
